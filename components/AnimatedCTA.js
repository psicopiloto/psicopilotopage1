...
export default function AnimatedCTA({ href, text, color = "green", className = "" }) {
  ...
  return (
    <Link href={href} passHref>
      <a
        className={`inline-block px-6 py-4 text-xl md:text-xl text-white rounded-2xl font-bold shadow-lg transform transition duration-300 ease-in-out hover:-translate-y-1 hover:scale-105 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-offset-2 ${colors[color]} ${className}`}
        role="button"
        aria-label={text}
      >
        {text}
